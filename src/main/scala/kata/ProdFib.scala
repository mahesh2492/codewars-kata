...
object ProdFib {
  ...
  private def fib: LazyList[Long] = {
    def tail(h: Long, n: Long): LazyList[Long] = h #:: tail(n, h + n)
  ...
  def productFib(prod: Long): Array[Long] = {
    val response =
      LazyList.from(1)
        ...
        .map(e => (e, e.product))
        .filter {
          case (_, product) => product >= prod
        }.take(1)
        .flatMap(_._1.toList).toList

    (response.product match {
      case `prod` => 1L :: response.reverse
      case _      => 0L :: response.reverse
    }).reverse.toArray
  }
